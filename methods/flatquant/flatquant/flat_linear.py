...
class FlatQuantizedLinear(nn.Module):
    def __init__(self, args, linear: nn.Linear, tp=False, act_quantizer=None):
        super(FlatQuantizedLinear, self).__init__()
        self.args = args
        self.linear = linear
        self.input_dim = self.linear.weight.shape[1]

        if tp:  # split weights along input dim
            self.num_groups = args.tp
            self.groupsize = self.input_dim // args.tp
        else:
            self.num_groups = 1
            self.groupsize = -1
        ...
        if self.lwc:
            ...
            init_value = 4.
            self.clip_factor_w_max = nn.Parameter(torch.ones((lwc_dim, self.num_groups, 1))*init_value, requires_grad=True)
            self.clip_factor_w_min = nn.Parameter(torch.ones((lwc_dim, self.num_groups, 1))*init_value, requires_grad=True)
            self.sigmoid = nn.Sigmoid()

        self._eval_mode = False

    def apply_wclip(self, weight):
        out_dim, inp_dim = weight.shape
        weight = weight.reshape(out_dim, self.num_groups, -1)
        ...
        weight = weight.reshape(out_dim, inp_dim)
        return weight
