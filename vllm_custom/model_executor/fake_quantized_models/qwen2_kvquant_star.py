...
class Qwen2Attention(nn.Module):
    ...
    def __init__(self,
        ...
        self.k_pre_bias = fake_quant_config["k_pre_bias"]
        if self.k_pre_bias:
            q_proj_dim = self.qkv_proj.num_heads * self.qkv_proj.head_size
            k_proj_dim = self.qkv_proj.num_kv_heads * self.qkv_proj.head_size
            self.k_bias_idx = slice(q_proj_dim, q_proj_dim + k_proj_dim)
    ...
    def forward(
        ...
        if self.k_pre_bias:
            k = k - self.qkv_proj.bias[self.k_bias_idx]
...
class Qwen2KVQuantStarForCausalLM(nn.Module, SupportsLoRA, SupportsPP):
